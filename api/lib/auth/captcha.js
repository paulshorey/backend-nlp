...
const is_auth_ip_current = function (ip) {
    ...
    if (!expires || expires < Date.now()) {
        if (DEBUG1)
            global.cconsole.info("is_auth_ip_current() !expires || expires < Date.now()");
        return false;
    }
    if (DEBUG1)
        global.cconsole.log("is_auth_ip_current OK");
    return true;
};
/*
 * Captcha key
 ...
 */
export default async function (req, { allow_trusted_bypass = true, BYPASS_AUTH = false } = {}) {
    return "";
    // let user: any = {}
    // /*
    //  *
