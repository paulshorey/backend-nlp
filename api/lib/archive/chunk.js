...
const mymodule = async function (string, tld = "com") {
    ...
    let options = { REBUILD, DEBUG_TIME };
    ...
    let chunks_rows = {}; // only real rows from DB get saved here
    /*
    ...
    if (string === "samiali")
        string = "sami ali";
    if (string === "hellosamiali")
        string = "hello sami ali";
    /*
     *
     *
     * STRING
    ...
    if (d1 && !isNaN(d1.index)) {
        for (let str of d1) {
            ...
            if (DEBUG1)
                global.cconsole.warn(`adding space to string "${string}" at index "${d1_i_start}"`);
            d1_i_start++;
            ...
            if (w) {
                if (w.index > 0) {
                    ...
                    if (DEBUG1)
                        global.cconsole.warn(`adding space to string "${string}" at index "${w.index}"`);
                }
            }
        }
    }
    ...
    let row_original = await str_row(string, { model: false, REBUILD: REBUILD });
    ...
    // time
    if (DEBUG_TIME)
        debug_time("lowest resolution - original phrase");
    if (DEBUG2)
        global.cconsole.debug("chunks after original", chunks_keys);
    /*
     *
     *
     *
    ...
    if (!row_original || !row_original.list_count || row_original.list_count <= 15) {
        // alt data
        let found = await spellcheck(string_original);
        // alt word
        ...
        if (DEBUG3)
            global.cconsole.warn("found spellcheck string = " + found.spellchecked);
        // alt chunk
        let row = await str_row(string, { REBUILD: REBUILD, model: false });
        if (DEBUG3 && row) {
    ...
    if (DEBUG3 || DEBUG_TIME)
        debug_time("bing spellcheck/autosuggest");
    if (DEBUG3 || DEBUG_TIME) {
    ...
    if (string_arr.length <= 1) {
        let broken = await wordbreak(string);
        if (broken.length > 1) {
            // sanitize
            ...
            for (let key of string_arr) {
                let row = await str_row(key, { REBUILD: REBUILD, model: true });
                if (row) {
                    chunks_rows[row.key] = row;
                }
            }
    ...
    if (DEBUG2)
        global.cconsole.debug("chunks after wordbreak", chunks_keys);
    /*
     *
     *
     *
    ...
    if (string_arr.length > 1 && chunks_keys.length <= 1) {
        ...
        for (let key of string_arr) {
            ...
            let row = await str_row(key, { REBUILD: REBUILD });
    ...
    // time
    if (DEBUG_TIME)
        debug_time("highest resolution - if chunked");
    if (DEBUG2)
        global.cconsole.debug("chunks after highest resolution", chunks_keys);
    /*
     *
     *
     *
    ...
    if (string_arr.length >= 3) {
        ...
        for (let i_to = string_arr.length - 1; i_to > 1; i_to--) {
            ...
            if (DEBUG2)
                global.cconsole.info(`middle resolution backward = "${key}"`);
            let row = await str_row(key, { REBUILD: REBUILD });
            // save only if its a good phrase! if enough synonyms
            if (row && row.list_count >= 10) {
                if (DEBUG1)
                    global.cconsole.success("middle resolution backward - added row", key);
                /*
                 * found subphrase!
                 */
        ...
        for (let i_from = 1; i_from < string_arr.length - 1; i_from++) {
            ...
            let key = words_slice.join(" ").trim();
            if (DEBUG2)
                global.cconsole.info(`middle resolution forward = "${key}"`);
            let row = await str_row(key, { REBUILD: REBUILD });
            // save only if its a good phrase! if enough synonyms
            if (row && row.list_count >= 10) {
                if (DEBUG1)
                    global.cconsole.success("middle resolution forward - added row", key);
                /*
                 * found subphrase!
                 */
                // if (row.list_count >= 25) {
    ...
    // time
    if (DEBUG_TIME)
        debug_time("middle resolution");
    if (DEBUG2)
        global.cconsole.debug("after middle resolution", string, chunks_keys);
    // debug
    if (DEBUG2)
        global.cconsole.debug("chunks_keys", chunks_keys);
    if (DEBUG2)
        global.cconsole.debug("string ", string);
    if (DEBUG2)
        global.cconsole.debug("string_arr ", string_arr);
    if (DEBUG2)
        global.cconsole.debug("chunks_rows ", Object.keys(chunks_rows));
    /*
     *
     *
    ...
    if (DEBUG1)
        global.cconsole.warn("final chunks output chunks_keys", chunks_keys);
    if (DEBUG1)
        global.cconsole.warn("final chunks output string_arr", string_arr);
    /*
     *
     * TLD row
    ...
    if (tld && !chunks_rows[tld]) {
        let trow = await str_row(tld, { REBUILD: REBUILD });
    ...
    if (REBUILD || REBUILD_END) {
        ...
        if (!chunks_keys.length) {
            if (string_arr.length) {
                ...
                if (!row) {
                    let row = await str_row(string, { REBUILD: true });
                    chunks_rows[row.key] = row;
                }
            }
        }
