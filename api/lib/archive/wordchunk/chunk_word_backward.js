...
let DEBUG1 = false;
let DEBUG2 = false;
const chunk_word_backward = async function (string, options = {}) {
    if (DEBUG2)
        global.cconsole.log('chunk backward "' + string + '"');
    /*
     * validate & filter
     */
    // set default options:
    ...
    if (DEBUG1)
        global.cconsole.info("chunk_word_backward()", string);
    ...
    for (let i_to = string.length; i_to >= MINLEN; i_to--) {
        ...
        if (DEBUG2)
            global.cconsole.log(str, row ? row.list_count : "row is undefined");
        // not real word?
        ...
        if (DEBUG2)
            global.cconsole.success("found chunk row", str, row.list_count, row.pos1, row.aux);
        /*
        ...
        if (str_remainder) {
            ...
            if (row_remainder && row_remainder.list_count >= options.MINSYN) {
                if (DEBUG2)
                    global.cconsole.warn("remainder chunk row", str_remainder, row_remainder.list_count);
                /*
