...
let DEBUG0 = false;
let DEBUG1 = false;
const thisModule = async function (string, super_options = {}) {
    // set parameters for this and child scripts:
    if (DEBUG1)
        global.cconsole.info(`chunk_word() "${string}"`);
    let MINSYN = 4;
    let options = {
        str_row_options: {
            model: false,
    ...
    try {
        ...
        if (options.found_backward.length || options.found_forward.length) {
            ...
            for (let word of options.found_backward) {
                let row = await str_row(word, options.str_row_options);
            ...
            if (DEBUG0)
                global.cconsole.warn(`                   ch1_str "${ch1_str}"`);
            // string corrupted ?
            if (ch1_str.includes(" ") &&
        ...
        if (DEBUG1)
            global.cconsole.debug(`after 1st try ((${ch1_score}))`, ch1_keys);
        if (DEBUG1)
            global.cconsole.debug(`str_forward="${ch1_forward_key}"`);
        if (DEBUG1)
            global.cconsole.debug(`str_backward="${ch1_backward_key}"`);
        /*
         * 2ND TRY
         */
        let ch2_rows = {};
        ...
        if (DEBUG0)
            global.cconsole.warn(`                   ch2_str "${ch2_str}"`);
        // string corrupted !!!???
        if (ch2_str.includes(" ") &&
        ...
        if (DEBUG1)
            global.cconsole.debug(`after 2nd try ((${ch2_score}))`, ch2_keys);
        if (DEBUG1)
            global.cconsole.debug(`str_forward="${ch2_forward_key}"`);
        if (DEBUG1)
            global.cconsole.debug(`str_backward="${ch2_backward_key}"`);
        // }
        /*
         * COMPARE 1ST AND 2ND
        ...
        if (!ch1_str_changed && ch2_string_changed) {
             ...
             */
            if (DEBUG1)
                global.cconsole.log("!ch1_str_changed && ch2_string_changed", Object.keys(ch1_rows));
             ...
             */
            if (DEBUG1)
                global.cconsole.log("ch1_str_changed && !ch2_string_changed", Object.keys(ch2_rows));
            ...
            if (ch1_score > ch2_score) {
                // choose 1st
                if (DEBUG1)
                    global.cconsole.log("ch1_score > ch2_score", Object.keys(ch1_rows));
                return { chunked_string: ch1_str, chunked_rows: ch1_rows };
            }
            else {
                // choose 2nd
                if (DEBUG1)
                    global.cconsole.log("ch1_score <= ch2_score", Object.keys(ch2_rows));
                return { chunked_string: ch2_str, chunked_rows: ch2_rows };
            }
        }
        else {
             ...
             */
            if (DEBUG1)
                global.cconsole.error("possibly_corrupted_output", Object.keys(ch2_rows));
