...
export default async function (word_arr = []) {
    ...
    {
        let row = await get_row(word_arr.join(""), output);
        if (row && row.key) {
    ...
    if (word_arr.length > 2) {
        for (let i = 0; i < word_arr.length - 1; i++) {
            ...
            let combined_row = await get_row(combined_key, output);
    ...
    // ex: ["unitedstates", "of", "america"] has "united"+"states"
    {
        for (let i = 0; i < word_arr.length; i++) {
            let row = await get_row(word_arr[i], output);
                    ...
                    for (let phi = 0; phi < word_arr.length; phi++) {
                        if (phi === i) {
                            ...
                            for (let word of cwords) {
                                let row = await get_row(word, output);
...
async function get_row(key, output) {
    ...
    if (!row) {
        ...
        row = await data_word_get_parsed(key, "key,str,root,singular,plural,pos1,pos2,pos3,sentiment,name,brand,ctr,dict,pos_short,poss,tlds,list,ok_list,list_count,ok_count,word_count,char_count");
        if (row) {
