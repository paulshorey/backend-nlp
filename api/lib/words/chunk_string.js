...
export default async function (input_str, tld, do_spellcheck) {
    if (DEBUG1)
        global.cconsole.log("api/lib/words/chunk_string", input_str, tld);
    let output_arr;
    let output_alts;
    ...
    if (DEBUG1)
        global.cconsole.log("AFTER wordbreak(), output_str = ", output_str);
    // parse str (if result is different than input, skip next step)
    // if (do_spellcheck) {
    let spelled_obj = await spellcheck(output_str);
    if (DEBUG1)
        global.cconsole.log("AFTER spellcheck(), spelled_obj = ", spelled_obj);
    if (spelled_obj) {
    ...
    if (!output_arr) {
        let broken = (await spellcheck_wordbreak_wordchunk(output_str, { spellcheck: false })) || {};
        output_arr = broken.chunks[0] || [];
    }
    if (DEBUG1)
        global.cconsole.log("AFTER wordbreak(), output_arr = ", output_arr);
    // tokenize and get word info about every chunk
    // output is: {chunks_rows<object>, chunks_keys<array>}
    let output = await wordchunk(output_arr);
    // fix
