...
export const data_domain_syns4 = function (key) {
    return new Promise(async (resolve) => {
        let queryString = `SELECT key FROM data.domains WHERE syns4 ILIKE e'%"${key.replace(/[']+/g, "''")}"%' ORDER BY rank DESC`;
        pool
            .query(queryString)
            .then((res) => {
            if (res.rows.length) {
...
export const data_domain_syns3 = function (key) {
    return new Promise(async (resolve) => {
        if (DEBUG_TIME)
            debug_time("");
        let queryString = `SELECT key FROM data.domains WHERE syns3 ILIKE e'%"${key.replace(/[']+/g, "''")}"%' ORDER BY rank DESC`;
        pool
            .query(queryString)
            .then((res) => {
            if (DEBUG_TIME)
                debug_time("data_domain_syns3");
            if (res.rows.length) {
                resolve(res.rows);
            }
            else {
...
export const data_domain_syns2 = function (key) {
    return new Promise(async (resolve) => {
        if (DEBUG_TIME)
            debug_time("");
        let queryString = `SELECT key FROM data.domains WHERE syns2 ILIKE e'%"${key.replace(/[']+/g, "''")}"%' ORDER BY rank DESC`;
        pool
            .query(queryString)
            .then((res) => {
            if (DEBUG_TIME)
                debug_time("data_domain_syns2");
            if (res.rows.length) {
                resolve(res.rows);
            }
            else {
...
export const data_domain_syns1 = function (key) {
    return new Promise(async (resolve) => {
        if (DEBUG_TIME)
            debug_time("");
        let queryString = `SELECT key FROM data.domains WHERE syns1 ILIKE e'%"${key.replace(/[']+/g, "''")}"%' OR syns ILIKE e'%"${key.replace(/[']+/g, "''")}"%' ORDER BY rank DESC`;
        pool
            .query(queryString)
            .then((res) => {
            if (DEBUG_TIME)
                debug_time("data_domain_syns1");
            if (res.rows.length) {
                resolve(res.rows);
            }
            else {
...
export const data_domains_get_parsed = function (columns, where = "") {
    return new Promise(async (resolve) => {
        let rows = await data_domains_get(columns, where);
        if (!rows || !rows.length) {
            resolve([]);
        }
        for (let row of rows) {
...
export const data_domain_get = function (key, columns) {
    let selectWhat = "*";
    if (columns) {
        selectWhat = columns.toString();
    }
...
export const data_domain_get_parsed = function (key, columns) {
    return new Promise(async (resolve) => {
        let row = await data_domain_get(key, columns);
...
export const data_domain_put = function (row) {
    if (DEBUG1)
        global.cconsole.log("data_word_put", row);
    if (DEBUG2)
        global.cconsole.log("data_word_put", [row.key]);
    return new Promise(async (resolve) => {
        // filter and validate inputs
        let { key } = row;
        if (!row.key) {
        ...
        key = key.replace(/[']+/g, "''");
        // add if not exist
        let existing = await data_domain_get(key, ["key"]);
        if (!existing) {
