...
export default [
    ...
    },
    {
        ...
        response: async function ({ res, req }) {
             ...
             * VALIDATE INPUTS
             */
            if (input_str === "test_fail")
                throw new Error("test failed on purpose - hello_worl");
             ...
             */
            let results = await get_domains_suggestions(response.string_original, response.string, response.tld, chunks.chunks_keys, chunks.chunks_rows, query, input_tlds_user || [], query.tlds_ignore || [], chunks.bing_alts || []);
            /*
             * RESPONSE
             */
            if (!response.tld) {
