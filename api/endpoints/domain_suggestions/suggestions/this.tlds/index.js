 ...
 */
let DEBUG1 = false;
export default async function make_tlds() {
    // generate tlds
    let { yes_tlds, maybe_tlds } = await tlds_from_chunks.call(this);
    /*
     *
     * sort
     *
     */
    if ((this.chunks_dict["black"] && this.chunks_dict["lives"]) ||
        ...
        if (DEBUG1)
            global.cconsole.log("yes_tlds sorted", yes_tlds);
        // maybe_tlds = sort_strings_by_rating_and_position(maybe_tlds, sort_preferences_all, 1)
        if (DEBUG1)
            global.cconsole.log("maybe_tlds sorted", maybe_tlds);
    }
    /*
     *
