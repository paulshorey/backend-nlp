// import original_replace_original from "@ps/nlp/api/endpionts/domain_suggestions/suggestions/this.phrase_lists/word_dict/original_replace_one"
// import original_one_word from "@ps/nlp/api/endpionts/domain_suggestions/suggestions/this.phrase_lists/word_dict/original_one_word"
...
export default function () {
    ...
    if (DEBUG1)
        global.cconsole.warn("key_words", keys_words);
    ...
    matrix.forEach((column, ci) => {
        ...
        if (row.root)
            derivations[row.root.toLowerCase()] = row.root.toLowerCase();
        if (row.singular)
            derivations[row.singular.toLowerCase()] = row.singular.toLowerCase();
        if (row.abbreviation) {
        ...
        if (row.acronym)
            derivations[row.acronym.toLowerCase()] = row.acronym.toLowerCase();
        // only allow plural for last word in phrase
        if (ci === matrix.length - 1) {
            if (row.plural)
                derivations[row.plural.toLowerCase()] = row.plural.toLowerCase();
        }
