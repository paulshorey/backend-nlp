...
export default function (doms_arr, { DEBUG_TIME = false } = {}) {
    return new Promise(async (resolve) => {
        ...
        let time_debug = function (message = "") {
            if (DEBUG_TIME) {
                console.log("DEBUG_TIME (cli_host) MANY (" + doms_arr.length + ")", ((Date.now() - time_start) / 1000).toFixed(3), message);
            }
        };
        // start
        let doms_code = {};
        ...
        cli_command += "} | cat";
        if (DEBUG1)
            global.cconsole.log(cli_command);
        /*
         * get whois
         */
