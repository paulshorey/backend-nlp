...
export default function (doms_arr, { DEBUG_TIME = false } = {}) {
    return new Promise((resolve) => {
        ...
        let time_debug = function (message = "") {
            if (DEBUG_TIME) {
                console.log("DEBUG_TIME (name) MANY (" + doms_arr.length + ")", ((Date.now() - time_start) / 1000).toFixed(3), message);
            }
        };
        // start
        let doms_dict = {};
        ...
        let data = { domainNames: doms_arr };
        axios
            .post(url, data)
            .then((response) => {
            if (DEBUG1)
                global.cconsole.log(typeof response.data.results, response.data.results);
