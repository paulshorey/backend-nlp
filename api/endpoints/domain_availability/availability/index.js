...
let USE_CACHE = true;
const availability = async function ({ domains = [], options }) {
    if (!options)
        options = {};
    // debug time
    ...
    for (let domname of domains) {
        ...
        if (b_domainr.length === 50) {
            if (DEBUG1)
                global.cconsole.log("fill bucket domainr", 50, b_domainr);
    ...
    // dump remaining content in bucket (what's left after last, full dump)
    {
        ...
        if (len) {
            if (DEBUG1)
                global.cconsole.log("fill bucket cli_host", len);
    ...
    }
    {
        ...
        if (len) {
            if (DEBUG1)
                global.cconsole.log("fill bucket cli_whois", len);
    ...
    }
    {
        ...
        if (len) {
            if (DEBUG1)
                global.cconsole.log("fill bucket domainr", len);
            requests.push(check_domainr(b_domainr, { DEBUG_TIME }));
        }
    }
    /*
     ...
     */
    if (DEBUG1)
        global.cconsole.log("n_from_localstorage", n_from_localstorage);
    /*
     * OK
     */
    i_skip++;
