...
export const get_word = function get_word(key, { keepCase } = {}) {
    return new Promise(async (resolve) => {
        key = key.replace(/[']+/g, "''");
        if (!keepCase) {
...
export const get_singular_of_key = function (key) {
    return new Promise(async (resolve) => {
        ...
        let queryString = `SELECT key, list_count FROM crawl.words WHERE (plural=e'${key.replace(/[']+/g, "''")}' OR plural=e'${key
            .toLowerCase()
            .replace(/[']+/g, "''")}') AND (list_count>=6 OR (sources_count>=4 AND list_count>=2)) ORDER BY list_count DESC LIMIT 1`;
        pool
            .query(queryString)
            .then((res) => {
            if (res.rows.length) {
...
export const get_ok_key = function (singular) {
    return new Promise(async (resolve) => {
        // plural input can be Capitalized ~ will check for exact string, and also lowercase version
        let queryString = `SELECT key, list_count FROM crawl.words WHERE (key=e'${singular.replace(/[']+/g, "''")}' OR key=e'${singular
            .toLowerCase()
            .replace(/[']+/g, "''")}') AND (list_count>=6 OR (sources_count>=4 AND list_count>=2)) ORDER BY list_count DESC LIMIT 1`;
        pool
            .query(queryString)
...
export const get_reverse_list_acronyms = function (key) {
    return new Promise(async (resolve) => {
        key = key.replace(/[']+/g, "''").toLowerCase();
        let queryString = `SELECT key, acronym, list_count, (length(sources) - length(replace(list, e'"${key}"', '')) / length(e'"${key.replace(/[']+/g, "''")}"')) AS occurrences FROM crawl.words WHERE list_count>1 AND key!=e'"${key.replace(/[']+/g, "''")}"' AND list ILIKE e'%"${key.replace(/[']+/g, "''")}"%' AND acronym IS NOT NULL AND acronym!='' ORDER BY occurrences DESC LIMIT 50`;
        pool
            .query(queryString)
            .then((res) => {
            if (res.rows.length) {
...
export const get_reverse_sources = function (key) {
    return new Promise(async (resolve) => {
        key = key.replace(/[']+/g, "''").toLowerCase();
        let queryString = `SELECT key, list_count, (length(sources) - length(replace(sources, e'"${key}"', '')) / length(e'"${key.replace(/[']+/g, "''")}"')) AS occurrences FROM crawl.words WHERE list_count>1 AND key!=e'"${key.replace(/[']+/g, "''")}"' AND sources ILIKE e'%"${key.replace(/[']+/g, "''")}"%' ORDER BY occurrences DESC LIMIT 50`;
        pool
            .query(queryString)
            .then((res) => {
            if (res.rows.length) {
...
export const get_words = function (offsetOrString, limit = 500) {
    ...
    return new Promise(async (resolve) => {
        ...
        let queryString = `SELECT * FROM crawl.words WHERE ` +
            (string ||
...
export const add_word = function (row) {
    return new Promise(async (resolve) => {
        ...
        try {
            ...
            let res = await pool.query(queryString);
            if (res && res.rows && res.rows.length) {
                // ignore
                resolve("");
                return;
        ...
        // add new row
        let queryString = `INSERT INTO crawl.words (key, attempts, vrsn, timestamp, char_count, word_count, rating) VALUES (e'${key}', 0, ${vrsn || null}, ${Date.now()}, ${char_count}, ${word_count}, ${rating})`;
        pool
            .query(queryString)
            .then((res) => {
...
export const set_word = function (query) {
    return new Promise(async (resolve) => {
        ...
        if (key !== query.key) {
            ...
            let lowercase_row = await get_word(key);
            let uppercase_row = await get_word(query.key, { keepCase: true });
