...
import { data_word_get_parsed } from "../pgdb";
 ...
 */
const thisModule = async function (str, options = {}) {
    ...
    return new Promise(async (resolve) => {
        try {
             ...
             *
             */
            let dbrow = await data_word_get_parsed(row.key);
            if (dbrow) {
                /*
                 * FOUND
                 */
                if (DEBUG1)
                    global.cconsole.success(`found row "${row.key}"`);
                row = dbrow;
            }
            else {
                /*
                 ...
                 */
                if (DEBUG1)
                    global.cconsole.warn(`!row "${row.key}"`);
                if (options.model === false) {
                    /*
                     * Return nothing if specified, though by default would continue with empty row
                     */
                    if (DEBUG1)
                        global.cconsole.log(`resolve empty object, because !row and options.model===false "${row.key}"`);
                    resolve({});
                    return;
                }
                else {
                     ...
                     */
                    if (DEBUG1)
                        global.cconsole.log(`resolve default row for "${row.key}"`);
                    row = row_model(row);
                    row_meta(row);
                    resolve(row);
                    return;
             ...
             */
            if (REBUILD) {
                if (DEBUG2 && !options.synonyms)
                    global.cconsole.warn(`rebuild row "${key}"`);
                if (DEBUG1 && options.synonyms)
                    global.cconsole.success("options.synonyms = ", options.synonyms);
                // time
                if (DEBUG_TIME)
                    debug_time("rebuild start " + str);
                /*
                ...
                if (options.synonyms) {
                    ...
                    if (DEBUG1)
                        global.cconsole.success("added synonyms", add_dict);
                }
                // time
                if (DEBUG_TIME)
                    debug_time("rebuilt added derivations to row.dict " + str);
                /*
                 * Compile data object
                 */
                if (DEBUG2)
                    global.cconsole.warn("rebuilding row data object... (tlds, meta, pos_short, poss)");
                row_pos_short(row, { DEBUG1: !!options.synonyms });
                if (DEBUG_TIME)
                    debug_time("rebuilt pos_short " + str);
                await row_poss(row, options);
                if (DEBUG_TIME)
                    debug_time("rebuilt poss " + str);
                row_meta(row); // modifies .poss if .proper
                if (DEBUG_TIME)
                    debug_time("rebuilt meta " + str);
                await row_tlds(row);
                if (DEBUG_TIME)
                    debug_time("rebuilt tlds " + str);
                row_food(row); // requires updated .tlds
                row_fw(row); // modifies .poss
                if (DEBUG_TIME)
                    debug_time("rebuilt food/fw " + str);
            }
            /*
             *
            ...
            if (REBUILD_TLDS) {
                if (DEBUG_TIME)
                    debug_time("rebuild tlds only start " + str);
                if (DEBUG2)
                    global.cconsole.warn("rebuilding row data object... (tlds only)");
                row = await row_tlds(row);
                if (DEBUG2)
                    global.cconsole.warn("after row_tlds");
                if (DEBUG_TIME)
                    debug_time("rebuilt tlds " + str);
            }
            /*
            ...
            if (REBUILD_LIST_COUNT) {
                if (DEBUG_TIME)
                    debug_time("rebuild list_count only start " + str);
                if (DEBUG2)
                    global.cconsole.warn("rebuilding row data object... (list_count/meta/stopwords only)");
                // stopword ? name ? brand
                row_meta(row);
                if (DEBUG2)
                    global.cconsole.warn("after row_tlds");
                if (DEBUG_TIME)
                    debug_time("rebuilt list_count/meta " + str);
            }
            /*
             *
             * Resolve
             ...
             */
            if (DEBUG2)
                global.cconsole.log("resolve " + typeof row);
            resolve(row);
        }
        catch (e) {
            global.cconsole.error("str_row() error!", e);
            resolve(undefined);
            return;
        }
    });
