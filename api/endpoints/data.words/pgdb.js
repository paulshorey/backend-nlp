...
export const data_word_delete = function (key) {
    return new Promise(async (resolve) => {
        ...
        pool
            ...
            .then(() => {
            if (DEBUG1)
                global.cconsole.warn("deleted ", key);
            resolve(true);
        })
            .catch((err) => {
            if (DEBUG1)
                global.cconsole.error(`could not delete "${key}"`, err);
            resolve(null);
        });
    });
};
...
export const data_key_str_unique = async function (row) {
    ...
    if (DEBUG2)
        global.cconsole.warn(`data_key_str_unique key "${row.key}" vs key_cleaned "${key_cleaned}"`);
    if (key_cleaned === row.key.toLowerCase()) {
        // no difference - continue
        if (DEBUG2)
            global.cconsole.warn("no difference - continue");
    ...
    let row_cleaned = await data_word_get(key_cleaned, "key,proper,acronym,abbreviation,list_count,conjunction");
    let row_old = await data_word_get(row.key, "key, proper, ctr, acronym, str, abbreviation, singular, plural, root,list_count,conjunction");
    ...
    if (!row_cleaned) {
        ...
        if (DEBUG2)
            global.cconsole.warn("no entry for cleaned version: continue", row.key);
        // row = row_meta(row) // fix meta (str, proper, acronym, key)
        return row;
    }
        ...
        if (DEBUG2)
            global.cconsole.warn("dirty key is better: remove cleaned", key_cleaned);
        ...
        if (DEBUG2)
            global.cconsole.warn("cleaned key is better: remove this one (with punctuation)", row.key);
...
export const data_word_get = function (str, columns = "", limit = 1000) {
    ...
    return new Promise(async (resolve) => {
        key = key.toLowerCase();
        let queryString = `SELECT ${selectWhat} FROM data.words WHERE key=e'${key.replace(/[']+/g, "''")}' ORDER BY list_count DESC LIMIT ${limit}`;
        pool
            .query(queryString)
            .then((res) => {
            if (res.rows.length) {
...
export const data_word_get_parsed = function (key, columns, limit) {
    if (DEBUG2)
        global.cconsole.log("data_word_get_parsed", key);
    return new Promise(async (resolve) => {
        let row = await data_word_get(key, columns, limit);
...
export const data_word_put = async function (row, REBUILD = false) {
    ...
    try {
        ...
        if (DEBUG2)
            global.cconsole.log(queryString);
        await pool.query(queryString);
    }
    catch (e) {
        // failed
...
export const data_word_sentiment_of_synonym = function (key, synonym, sentiment) {
    if (DEBUG1)
        global.cconsole.warn("data_word_sentiment_of_synonym", key, synonym, sentiment);
    return new Promise(async (resolve) => {
        // validate
...
export const data_word_remove_words = function ({ key, words }) {
    if (DEBUG1)
        global.cconsole.warn("data_word_remove_words", { key, words });
    return new Promise(async (resolve) => {
        // validate
        ...
        if (DEBUG1)
            global.cconsole.warn("deleted", row);
        resolve(true);
    });
};
export const data_word_add_poswords = function ({ key, pos, poswords: new_words }) {
    let DEBUG1 = false;
    if (DEBUG1)
        global.cconsole.warn("data_word_add_poswords", { key, pos, new_words });
    return new Promise(async (resolve) => {
        // validate
        if (!key || !pos || !Array.isArray(new_words)) {
            if (DEBUG1)
                global.cconsole.error("!key || !pos || !Array.isArray(new_words), exit");
            resolve(false);
        }
        // get row
        let row = await data_word_get_parsed(key);
        if (!row) {
            if (DEBUG1)
                global.cconsole.error("!row, exit");
            resolve(false);
        }
        /*
        ...
        for (let word of new_words) {
            if (word) {
                ...
                try {
                    ...
                    if (DEBUG1)
                        global.cconsole.success(`found row for word="${word}"`);
                }
                catch (e) {
         ...
         */
        if (DEBUG1)
            global.cconsole.success(row.key);
        if (DEBUG1)
            global.cconsole.success(row.pos1);
        for (let key in row.dict) {
            if (new_words.includes(key)) {
                if (DEBUG1)
                    global.cconsole.success(key, row.dict[key]);
            }
        }
        /*
         * Save final edited row
        ...
        if (DEBUG1)
            global.cconsole.warn("final row.dict[word] is ...", row.dict && row.dict[(new_words || [])[0]]);
        resolve(true);
    });
};
