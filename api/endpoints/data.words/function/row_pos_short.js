...
let DEBUG1 = false;
let DEBUG2 = false;
export default function (row = {}, options = {}) {
    // debug
    if (!!options.DEBUG2) {
        DEBUG2 = true;
    }
    ...
    for (let pos in row.pos_short) {
        if (DEBUG1)
            global.cconsole.info("pos", pos);
        if (DEBUG1)
            global.cconsole.info("row.pos_short[pos]", row.pos_short[pos || ""]);
        // only use first 5-15
        // 1. if DB mentions how many to use, follow that!
        // 2. use HALF of the synonyms, or 15, whichever is greater
        let limit = Math.min(row[`x${pos}`] || (pos_i === 0 ? 15 : Math.max(Math.min(15, Math.floor(row.pos_short[pos].length / 2)), 5)));
    ...
    for (let pos in row.pos_short) {
        ...
        for (let iword in row.pos_short[pos]) {
            ...
            if (DEBUG1)
                global.cconsole.log(word, [list_count, length, ratings[word]]);
