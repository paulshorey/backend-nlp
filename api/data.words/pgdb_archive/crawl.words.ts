...
export const increment_stopwords = function increment_stopwords(key, attempt = 1) {
  ...
  return new Promise(async (resolve) => {
    ...
    if (attempt > 10) {
      global.ccconsole.error("increment_stopwords failed to write 10 times in a row!")
      resolve(null)
    }
  ...
  return new Promise(async (resolve) => {
    ...
    pool
      ...
      .then((res) => {
        ...
        global.ccconsole.info(`added "${key}" ${withRating}`)
        resolve(key)
      })
      .catch((err) => {
        global.ccconsole.warn(`tried to add "${key}"`)
        global.ccconsole.error(err)
        resolve(err)
      })
  })
    ...
    if (key !== query.key) {
      ...
      if (lowercase_row && uppercase_row) {
        ...
        if (lowercase_row.list_count > uppercase_row.list_count) {
          global.ccconsole.warn(["deleting uppercase", uppercase_row.key, "keeping lowercase", lowercase_row.key])
          await delete_word(uppercase_row.key) // delete UpperCase 'Key' row
          key = query.key = lowercase_row.key // keep lowercase key
        } else {
          global.ccconsole.warn(["deleting lowercase", lowercase_row.key, "keeping uppercase", uppercase_row.key])
        ...
        // if no duplicate row
        global.ccconsole.warn(["fixing case key", query.key, "to", key])
...
export const delete_word = function delete_word(key) {
  global.ccconsole.error('deleting key = "' + key + '"')
