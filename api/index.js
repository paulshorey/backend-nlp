...
if (!host_is_dev) {
    try {
        const ssl_key = fs.readFileSync(global.__root + `/_certs/api${process.env.PRODUCTION ? "" : "-staging"}.wordio.co.key`, "utf8");
        const ssl_crt = fs.readFileSync(global.__root + `/_certs/api${process.env.PRODUCTION ? "" : "-staging"}.wordio.co.crt`, "utf8");
